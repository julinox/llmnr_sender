...
PUBLIC NETIFIPV6 *searchIpv6(IN6ADDR *ip, NETIFACE *list)
{
        NETIFIPV6 *ip6;
        NETIFACE *current;
        ...
        for (; current != NULL; current = current->next) {
                ip6 = NULL;
                ip6 = getNetIfIpv6Node(current,ip);
                if (ip6 != NULL)
                        break;
        }
        return ip6;
...
PUBLIC NETIFIPV4 *searchIpv4(INADDR *ip, NETIFACE *list)
{
        NETIFIPV4 *ip4;
        NETIFACE *current;
        ...
        for (; current != NULL; current = current->next) {
                ip4 = NULL;
                ip4 = getNetIfIpv4Node(current,ip);
                if (ip4 != NULL)
                        break;
        }
        return ip4;
...
PUBLIC IN6ADDR *getFirstValidAddr6(NETIFACE *iface)
{
        NETIFIPV6 *current;
